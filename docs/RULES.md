...
| H15 | **Daylight-only subjects.** Sport cannot run after **17:00** - the stadium has no lighting. With 10 periods of one hour from 08:00, 17:00 is the end of **period 9**, so Sport may occupy periods 1-9 and never period 10. **Corrected 2026-08-25** (Majd: "no sport four to five is fine, it cant go to six") - the earlier period-8 reading was one hour too strict and made last year's real table look illegal 9 times. Stated by Majd 2026-08-24: "sport time window daylight meaning morning and max 14h to 16h". Generalised: any subject may carry a `latest_period` in the Subjects sheet. | VERIFIED |
...
| 2026-08-25 | **Test C4 PASSED** (Majd's screenshot: OT1 and OT2 both show the two options stacked in one period) - the H14 multi-class option-lesson format is proven in the real aSc. **Test C3 v1 did NOT clear the phantom copies**; last year's real export revealed the answer: aSc 2013 encodes fortnightly lessons as fractional periodsperweek (their file has periodsperweek="0.5" 63 times, lesson-level weeksdefid on 104 A/B lessons). emit_asc now writes cards/2 for week-A/B lessons; testC3 updated to v2 (0.5/1.0/1.0) for one more import. **HARD promotion built** (Majd: "what if we stuff more hard rules... build right is easier than maintain later"): any weight in the Weights sheet set to HARD/صارم becomes an unbreakable constraint instead of a score - smoke-proven (soft=OPTIMAL, HARD=INFEASIBLE on a forced conflict); extra_day_present is blacklisted; phase 1 reports plainly when too many HARD rules leave no timetable. Selftest 23/23. |
| 2026-08-25 | **Test C3 v2 PASSED** (Majd's screenshot: cards placed, unplaced strip EMPTY) - fractional periodsperweek closes the weeks story. Every aSc format the solver emits is now proven by import in the real aSc 2013: day masks, split groups, week A/B, pooled options. The code side of the project is COMPLETE; what remains is data (days off, training days, sizes, home rooms, zones, Options class lists, the 9 Distribution flags), the curriculum regeneration decision, the ALT confirmation, weight tuning with Majd, and the long runs. |
| 2026-08-25 | **Majd's late-night rule dump, recorded verbatim and triaged.** CODED as data: م ه آلية / م ه كهـ are EXCLUSIVE engineering labs (new room types eng_mech/eng_elec, MECH/ELEC subjects retyped); تقنية 1/2 = normal TECH only, cap 20; normal ق rooms hold 40; Group room = normal-but-small cap 20; labs hold half a class (فيز1 cap 20), فيز2/فيز3 can hold a full class (cap 40, UNSURE-maybe), علوم1/علوم2 cap 30 and 'try to avoid' full classes there (UNSURE-maybe); Inf rooms cap 20 - it/tech/gym rooms serve ONLY their subject's TP; physics and SVT labs MAY host normal lessons (capacity permitting); small classes fitting the Group room, sizes set to 20 flagged UNSURE ('were few'): 3آداب1, 3رياضيات1, 3علوم تقنية1/2, 4آداب1, 4رياضيات1. Day off stays flexible. QUEUED FEATURES (next build, explicitly promised): (1) fortnight week A/B becomes the SOLVER'S choice per row ('week a and b are something u choose not rules - the rule is once in two weeks'); (2) per-room CAPACITY matching in the model+assigner (today capacity is data only - the pooled counting still uses the small-class<=24 bar); (3) alternative block patterns ('we bent the guide for English and economy from 2+1+1 to 2+2 cause it was hard'). Training days: to come from the official ministry file - location pending Majd's pointer (the enrollment list has no such column). |
| 2026-08-25 | **THE DUEL RESULT — last year's school, same data, same ruler.** Solver, 2h strict, independently verified **ALL GREEN** (zero hard-rule violations, zero exceptions): teacher hole-hours **11.5** vs 35.0 by hand · pupil hole-hours **0.0** vs 23.5 · lessons in the last period **140** vs 139 · teacher one-hour days **2.0** vs 0.5 (the only metric still behind: 9 cases, of which 4 are one teacher's fortnightly rows). Caveats kept honest: the hand file is a WORK-IN-PROGRESS copy (Majd's final was better), and the rebuilt data carries 1670 cards vs 1682 (12 unparseable cards + 3 nested-option bindings released, all flagged at extraction). Conclusion: the machine beats the hand-made draft decisively on pupils and teacher gaps, ties on late periods, and trails slightly on lone-hour days. |
